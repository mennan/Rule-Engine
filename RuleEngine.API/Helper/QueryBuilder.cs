...
namespace RuleEngine.API
{
    public static class QueryBuilder
    {
        ...
        {
            string parsedQuery;
            return BuildQuery4(queryable.AsQueryable(), filterRule, out parsedQuery, useIndexedProperty, indexedPropertyName);
        }
        ...
        /// <param name="indexedPropertyName">The indexable property to use</param>
        /// <returns>Filtered IQueryable</returns>
        public static IQueryable<T> BuildQuery2<T>(this IList<T> queryable, FilterRule filterRule, bool useIndexedProperty = false, string indexedPropertyName = null)
        {
            string parsedQuery;
            return BuildQuery4(queryable.AsQueryable(), filterRule, out parsedQuery, useIndexedProperty, indexedPropertyName);
        }
        ...
        /// <param name="indexedPropertyName">The indexable property to use</param>
        /// <returns>Filtered IQueryable</returns>
        public static IQueryable<T> BuildQuery3<T>(this IQueryable<T> queryable, FilterRule filterRule, bool useIndexedProperty = false, string indexedPropertyName = null)
        {
            string parsedQuery;
            return BuildQuery4(queryable, filterRule, out parsedQuery, useIndexedProperty, indexedPropertyName);
        }
        ...
        /// <param name="indexedPropertyName">The indexable property to use</param>
        /// <returns>Filtered IQueryable.</returns>
        public static IQueryable<T> BuildQuery4<T>(this IQueryable<T> queryable, FilterRule filterRule, out string parsedQuery, bool useIndexedProperty = false, string indexedPropertyName = null)
        {
            if (filterRule == null)
            {
                parsedQuery = "";
        ...
        private static Expression BuildExpressionTree(ParameterExpression pe, FilterRule rule, bool useIndexedProperty = false, string indexedPropertyName = null)
        {
            ...
            if (rule.Field != null)
            {
                ...
                else
                {
                    ...
                    {
                        propertyExp = Expression.Property(propertyExp ?? pe, field);
                    }
                }
        ...
        {
            var isNullable = !propertyExp.Type.GetTypeInfo().IsValueType || Nullable.GetUnderlyingType(propertyExp.Type) != null;
        ...
        private static Expression IsEmpty(Expression propertyExp)
        {
            ...
            if (IsGenericList(propertyExp.Type))
            {
                ...
                exOut = Expression.Property(propertyExp, propertyExp.Type.GetTypeInfo().GetProperty("Count"));

                exOut = Expression.AndAlso(nullCheck, Expression.Equal(exOut, someValue));
            }
            else
            {
                exOut = Expression.Property(propertyExp, typeof(string).GetTypeInfo().GetProperty("Length"));
        ...
        private static Expression Contains(Type type, string value, Expression propertyExp)
        {
            ...
            var method = propertyExp.Type.GetTypeInfo().GetMethod("Contains", new[] { type });

            Expression exOut = Expression.Call(propertyExp, typeof(string).GetTypeInfo().GetMethod("ToLower", Type.EmptyTypes));
        ...
        private static Expression EndsWith(Type type, string value, Expression propertyExp)
        {
            ...
            var method = propertyExp.Type.GetTypeInfo().GetMethod("EndsWith", new[] { type });
        ...
        private static Expression BeginsWith(Type type, string value, Expression propertyExp)
        {
            ...
            var method = propertyExp.Type.GetTypeInfo().GetMethod("StartsWith", new[] { type });
        ...
        private static Expression Equals(Type type, string value, Expression propertyExp)
        {
            ...
            if (type == typeof(string))
            {
                ...
                exOut = Expression.Call(propertyExp, typeof(string).GetTypeInfo().GetMethod("ToLower", Type.EmptyTypes));
                someValue = Expression.Call(someValue, typeof(string).GetTypeInfo().GetMethod("ToLower", Type.EmptyTypes));
                exOut = Expression.AndAlso(nullCheck, Expression.Equal(exOut, someValue));
            }
            else
            {
        ...
        private static Expression In(Type type, string value, Expression propertyExp)
        {
            ...
            if (IsGenericList(propertyExp.Type))
            {
                var genericType = propertyExp.Type.GetTypeInfo().GetGenericArguments().First();
                var method = propertyExp.Type.GetTypeInfo().GetMethod("Contains", new[] { genericType });
                Expression exOut;
                ...
                if (someValues.Count > 1)
                {
                    if (type == typeof(string))
                    {
                        ...
                        while (counter < someValues.Count)
                        {
                            exOut = Expression.Or(exOut,
                                Expression.Equal(
                                    Expression.Call(propertyExp, typeof(string).GetTypeInfo().GetMethod("ToLower", Type.EmptyTypes)),
